...
key="traffic-vision/$d/"; clips=`aws s3 ls s3://s3.kevino.ca/$key | grep ".mov$" | awk '{print $4}'`; for i in $clips; do aws s3 rm s3://s3.kevino.ca/$key$i & done

# on prodweb, insert a new video definition

insert into tv_point (lat,lon,title,streetview) values (null,null,'Bank and Isabella','');
insert into tv_video (point,recorded) values (5,'2016-09-19');
