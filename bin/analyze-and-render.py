...
def processframe(frameNum,frame,cap,watchPoints):
    ...
    cv2.putText(frame, "time:%0.1f frame:%d file:%s" % ((cap.get(cv2.cv.CV_CAP_PROP_POS_MSEC)/1000),frameNum,file_tag), 
    ...
    for i, w in enumerate(watchPoints):
        ...
        if True:
            pix = frame[w.y,w.x]
            pixFLOAT = frame[w.y,w.x].astype(np.float64)
            bgRatio = abs((pixFLOAT[0] / pixFLOAT[1]) - 1)
            print "%d :: %d :: (red:%d) bgr: %s bgratio: %.2f" % (frameNum,i,pix[2],pix,bgRatio)
            w.isRed = 0
        ...
        if w.isRed:
            col = (0,0,255)
        cv2.rectangle(frame,(w.x-20,100),(w.x+20,w.y-10),col,-1)
        if w.isRed:
            redFor = cap.get(cv2.cv.CV_CAP_PROP_POS_MSEC) - w.redStart
            cv2.rectangle(frame,(w.x+20,w.y-120),(w.x+300,w.y-10),(0,0,0),-1)
            cv2.putText(frame, "%0.1f" % (redFor/1000), 
                    (w.x+30,w.y-30), cv2.FONT_HERSHEY_DUPLEX, 3, (0,0,255,255), 4)
